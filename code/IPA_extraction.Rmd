...
path.dir <- file.path("../outs/ipa")
path <- file.path(path.dir, "ipa_txt")
IPA <- list.files(file.path( path), pattern = ".txt")
...
```{r}
outs_path <- file.path(path.dir, "results/core_analysis_outs")
...
# X <- lapply(X, function(y) factor(y$"Ingenuity Canonical Pathways", levels="Ingenuity Canonical Pathways"))
...
# filter #
#ur_sub <- lapply(ur, filter, "P-value" <= pval & abs(`Activation z-score`) > 2) 
...
#ur$v1_helminth_con_nutrition$`Activation z-score` <- rep(0)

ur <- lapply(ur, function(x) as.data.frame(x))
...
```{r, ipa_ssdim}
outs$infected_primed_vs_unprimed_epi_lung_only <- NULL
ssdim(outs);
```
...
#  dev.off()
#  
path.1 <-  file.path("../outs/ipa/results/core_analysis_outs/multienrichjam")
png(file.path(path.1, "compare_genes_from_genesets_bubbleplot.png"), width=9, height=9, pointsize=6, units="in", res=300)
mem_enrichment_heatmap(mem_canonical,
   p_cutoff=0.05);
dev.off()

svg(file.path(path.1, "compare_genes_from_genesets_bubbleplot.svg"), width=9, height=9, pointsize=6)
mem_enrichment_heatmap(mem_canonical,
   p_cutoff=0.05);
dev.off()
...
png(file.path(path.1, "compare_genes_from_genesets_heatmap.png"), width=9, height=9, pointsize=6, units="in", res=300)
mem_enrichment_heatmap(mem_canonical,
   style="heatmap",
   p_cutoff=0.05);
dev.off()

svg(file.path(path.1, "compare_genes_from_genesets_heatmap.svg"), width=9, height=9, pointsize=6)
mem_enrichment_heatmap(mem_canonical,
   style="heatmap",
   p_cutoff=0.05);
dev.off()
